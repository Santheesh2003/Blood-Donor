
var data={
  AndhraPradesh:[
            "Anantapur",
             "Chittoor",
             "East Godavari",
             "Guntur",
             "Krishna",
             "Kurnool",
             "Nellore",
             "Prakasam",
             "Srikakulam",
             "Visakhapatnam",
             "Vizianagaram",
             "West Godavari",
             "YSR Kadapa"
],
  "TamilNadu":{
             "Ariyalur":[
                "All",
                "Andimadam",
                "Ariyalur",
                "Sendurai",
                "Udayarpalayam"
             ],
             "Chennai":[
                "All",
                "Adambakkam",
                "Adyar",
                "Alwarpet",
                "Ambattur",
                "Anna Nagar",
                "Annanagar",
                "Annasalai",
                "Ashok Nagar",
                "Avadi",
                "Ayanavaram",
                "Besant Nagar",
                "Broadway",
                "Chepauk",
                "Chetput",
                "Chitlepakkam",
                "Choolaimedu",
                "Chrompet",
                "Dr.radhakrushnan Nagar",
                "Egmore",
                "Ennore",
                "Guindy",
                "Harbour",
                "Jaffarkhanpet",
                "Jawahar Nagar",
                "Kamaraj Nagar",
                "Kilpauk",
                "Kodambakkam",
                "Kodambakkam",
                "Kolathur",
                "Korattur",
                "Kottupuram",
                "Madhavaram",
                "Madipakkam",
                "Mambalam",
                "Manali",
                "Manapakkam",
                "Minambakkam",
                "Mylapore",
                "Nandambakkam",
                "Nandanam",
                "Nanganallur",
                "Nungambakkam",
                "Padi",
                "Pallavaram",
                "Pammal",
                "Park Town",
                "Perambur",
                "Porur",
                "Purasawalkam",
                "Ramapuram",
                "Saidapet",
                "Tambaram",
                "Theagaraya Nagar",
                "Thiruvanmiyur",
                "Thiruvottiyur",
                "Thousand Lights",
                "Tirsulam",
                "Tondiarpet",
                "Triplicane",
                "Vadapalani",
                "Vandalur",
                "Velacherry",
                "Villiwakkam",
                "Virugambakkam",
                "Washermanpet"
             ],
             Coimbatore:[
                "All",
                "Avinashi",
                "Coimbatore",
                "Dharapuram",
                "Kinathukkadavu",
                "Mettupalayam",
                "Palladam",
                "Perur",
                "Pollachi",
                "Pongalur",
                "Singanallur",
                "Thondamuthur",
                "Trippur",
                "Udumalaipet",
                "Valparai"
             ],
             Cuddalore:[
            "All",
            "Bhuvanagiri",
            "Chidambaram",
            "Cuddalore",
            "Kattumannarkoil",
            "Kurinjipadi",
            "Mangalore",
            "Nellikuppam",
            "Neyveli",
            "Panruti",
            "Virudhachalam"
             ],
             Dharmapuri:[
                "All",
                "Dharmapuri",
                "Harur",
                "Morrapur",
                "Palacode",
                "Pennagaram"
             ],
             Dindigul:[
                "All",
                "Athoor",
                "Dindigul",
                "Natham",
                "Nilakottai",
                "Oddanchatram",
                "Palani",
                "Vedasandur"
             ],
             "Erode":[
                "All",
                "Anthiyur",
                "Bhavani",
                "Bhavanisagar",
                "Erode",
                "Gobichettipalayam",
                "Kangayam",
                "Modakurichi",
                "Perundurai",
                "Sathyamangalam",
                "Vellakoil"
             ],
             "Kanchipuram":[
                "All",
                "Acharapakkam",
                "Alandur",
                "Chengalpattu",
                "Kancheepuram",
                "Maduranthakam",
                "Sriperumbadar",
                "Tambaram",
                "Tiruporur",
                "Uthiramerur"
             ],
             "Kanyakumari":[
                "All",
                "Colachel",
                "Kanniyakumari",
                "Killiyur",
                "Nagarcoil",
                "Padmanabhapuram",
                "Thiruvattar",
                "Vilvancode"
             ],
             "Karur":[
                "All",
                "Aravakurichi",
                "Karur",
                "Krishnarayapuram",
                "Kulithalai"
             ],
             "Killiyur":[
                "All",
                "Killiyur"
             ],
             "Krishnagiri":[
                "All",
                "Bargur",
                "Hosur",
                "Kavaeripattinam",
                "Krishnagiri",
                "Thalli"
             ],
             "Madurai":[
            "All",
            "Madurai",
            "Melur",
            "Samayanallur",
            "Sedapatti",
            "Sholavandan",
            "Thirumangalam",
            "Tiruparankundram",
            "Usilampatti"
             ],
             "Nagapattinam":[
                "All",
                "Kuttalam",
                "Mayiladuthurai",
                "Nagapattinam",
                "Poompuhar",
                "Sirkali",
                "Vedaranyam"
             ],
             "Namakkal":[
            "All",
            "Kabilarmalai",
            "Namakkal",
            "Rasipuram",
            "Sankagiri",
            "Sendamangalam",
            "Tiruchengode"
             ],
             "Nilgiris":[
                "All",
                "Coonoor",
                "Gudalur",
                "Ooatacamund"
             ],
             "Perambalur":[
                "All",
                "Alathur",
                "Jayankondam",
                "Kunnam",
                "Perambalur",
                "Varahur",
                "Veppanthattai"
             ],
             "Pudukkottai":[
                    "All",
                    "Alangudi",
                    "Aranthangi",
                    "Kolathur",
                    "Pudukkottai",
                    "Thirumayam"
             ],
             "Ramanathapuram":[
            "All",
            "Kadaladi",
            "Mudukulathur",
            "Paramakudi",
            "Ramanathapuram",
            "Thiruvadanai"
             ],
             "Salem":[
                "All",
                "Athur",
                "Edapadi",
                "Mettur",
                "Omalur",
                "Panamarathupatty",
                "Salem",
                "Talavasal",
                "Taramangalam",
                "Veerapandi",
                "Yercaud"
             ],
             "Sivaganga":[
                "All",
                "Ilayangudi",
                "Karikudi",
                "Manamadurai",
                "Sivaganga",
                "Tiruppathur"
             ],
             "Thanjavur":[
                "All",
                "Kumbakonam",
                "Orathanad",
                "Papanasam",
                "Thanjavur",
                "Thiruvidaimaruthur",
                "Thiruvonam",
                "Tiruvaiyaru"
             ],
             "Theni":[
                "All",
                "Andipatti",
                "Bodinayakkanur",
                "Cumbum",
                "Periyakulam",
                "Theni"
             ],            
             "Tiruchirappalli":[
                "All",
                "Lalkudi",
                "Marungapuri",
                "Musiri",
                "Srirangam",
                "Thiruverumbur",
                "Thottiam",
                "Tiruchirappalli",
                "Uppiliapuram"
             ],
             "Thiruvarur":[
            "All",
            "Mannargudi",
            "Nannilam",
            "Tiruthuraipoondi",
            "Tiruvarur",
            "Valangiman"
             ],
             "Tirunelveli":[
                "All",
                "Alangulam",
                "Ambasamudram",
                "Cheranmahadevi",
                "Kadayanallur",
                "Nanguneri",
                "Palayamkottai",
                "Puliyangudi",
                "Radhapuram",
                "Sankaranayanarkoil",
                "Tenkasi",
                "Tirunelveli",
                "Vasudevanallur"
             ],
             "Tirupur":[
                "All",
                "Dharapuram",
                "Kangeyam",
                "Nallur",
                "Palladam",
                "Tirupur",
                "Udumalaipettai",
                "Velampalayam",
                "Vellakoil"
             ],
             "Tiruvallur":[
                "All",
                "Gummidipoondi",
                "Pakkipet",
                "Ponneri",
                "Poonamallee",
                "Thiruttani",
                "Thiruvallur",
                "Thiruvottiyur",
                "Villivakkam"
             ],
             "Tiruvannamalai":[
                "All",
                "Arni",
                "Chengam",
                "Cheyyar",
                "Kalasapakkam",
                "Peranamallur",
                "Polur",
                "Thandarambattu",
                "Tiruvannamalai",
                "Vandavasi"
             ],
             "Tuticorin":[
                "All",
                "Kayalpatnam",
                "Koilpatti",
                "Ottappidaram",
                "Sattangulam",
                "Srivaikuntam",
                "Tiruchendur",
                "Tuticorin",
                "Vilathikulam"
             ],          
             "Vellore":[
                "All",
                "Vellore"
             ],
             "Viluppuram":[
            "All",
            "Chinnasalem",
            "Gingee",
            "Kandamangalam",
            "Melmalayanur",
            "Mugaiyur",
            "Rishivandiam",
            "Sankarapuram",
            "Thirunavalur",
            "Tindivanam",
            "Tirukoilur",
            "Ulundurpet",
            "Vanur",
            "Villupuram"
             ],
             "Virudhunagar":[
                "All",
            "Aruppukottai",
            "Rajapalayam",
            "Sattur",
            "Sivakasi",
            "Srivillithur",
            "Virudhunagar"
             ]
          
            }
}

window.onload = function(){
    const selectState=document.getElementById('state'),
          selectDistrict=document.getElementById('district'),
          selectCity=document.getElementById('city'),
          select=document.querySelectorAll('select');
          
         //  window.location="result.html";
     selectDistrict.disabled=true;
     selectCity.disabled=true;
     select.forEach(select => {
        if(select.disabled == true)
        {
            select.style.cursor="auto"
        }
     })
     for(let state in data) {
        console.log(state);
         selectState.options[selectState.options.length]=new Option(state, state)
     }
     selectState.onchange = (e) =>{
        selectDistrict.disabled=false;
        selectCity.disabled=true;

        selectDistrict.length=1
        selectCity.length=1

        for(let district in data[e.target.value]){
            console.log(district);
            selectDistrict.options[selectDistrict.options.length]=new Option(district,district)
        }
    }
    selectDistrict.onchange = (e) =>{
         selectCity.disabled=false;

         selectCity.length=1

          let city = data[selectState.value][e.target.value];
            // console.log(city);
         for(let i=0;i<city.length;i++){
            console.log(city);
            selectCity.options[selectCity.options.length]=new Option(city[i],city[i])
         }
      }
   // selectDistrict.onchange = (e) =>{
   //         selectCity.disabled=false;
           
   //          selectCity.length=1
   //          for(let city in data[selectState.value][e.target.value]){
   //             console.log(city);
   //             selectCity.options[selectCity.options.length]=new Option(city,city)
   //             // document.getElementById(selectCity).innerHTML=city;
   //          }
   // }
}
